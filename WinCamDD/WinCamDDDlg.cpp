...
BEGIN_MESSAGE_MAP(CWinCamDDDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_CAMERA0, &CWinCamDDDlg::OnBnClickedCamera0)
	ON_BN_CLICKED(IDC_CAMERA1, &CWinCamDDDlg::OnBnClickedCamera1)
	ON_BN_CLICKED(IDC_CAMERA2, &CWinCamDDDlg::OnBnClickedCamera2)
	ON_BN_CLICKED(IDC_FLOGTAKE, &CWinCamDDDlg::OnBnClickedFlogtake)
	ON_BN_CLICKED(IDC_USE_PROFILE, &CWinCamDDDlg::OnBnClickedUseProfile)
	ON_BN_CLICKED(IDC_QR_READ, &CWinCamDDDlg::OnBnClickedQrRead)
END_MESSAGE_MAP()
...
BOOL CWinCamDDDlg::OnInitDialog()
{
	...
	// TODO: 初期化をここに追加します。
	// Move dialog box to the right upper corner of screen.
	RECT rScreen, rDlg;
	GetWindowRect(&rDlg);	// Get size of dialog box.
	rScreen.left = 0;
	rScreen.right = GetSystemMetrics(SM_CXSCREEN);	// Get screen size.
	rScreen.top = 0;
	rScreen.bottom = GetSystemMetrics(SM_CYSCREEN);
	MoveWindow(rScreen.right - rDlg.right, 0, rDlg.right, rDlg.bottom, 1);
	...
	// Setup for camera.
	CheckDlgButton(IDC_CAMERA0, BST_CHECKED);
...
void CWinCamDDDlg::OnBnClickedCamera0()
{
	CAM_SelectCamera(0);
}


void CWinCamDDDlg::OnBnClickedCamera1()
{
	CAM_SelectCamera(1);
}


void CWinCamDDDlg::OnBnClickedCamera2()
{
	CAM_SelectCamera(2);
}


void CWinCamDDDlg::OnBnClickedFlogtake()
{
	fLogtake = TRUE;
}


void CWinCamDDDlg::OnBnClickedUseProfile()
{
	if (IsDlgButtonChecked(IDC_USE_PROFILE)) {
		fUseProfile = TRUE;
	}
	else {
		fUseProfile = FALSE;
...
void CWinCamDDDlg::OnBnClickedQrRead()
{
//	fQRread = !fQRread;
	fQRread = IsDlgButtonChecked(IDC_QR_READ);
}
